...
**Goal:** Create a lightweight `AttributesGuard` that can perform basic attribute validation for ABAC.

**File to create:** `backend/src/modules/auth/adapters/outbound/auth/attributes.guard.ts`
...
export class AttributesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const identity: RequestIdentity = request.identity || request.user; // fallback to user
...
**Commit message:** `feat: add AttributesGuard for ABAC`
