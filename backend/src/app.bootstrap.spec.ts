...
import { TypeOrmUserSession } from './modules/auth/adapters/outbound/persistence/typeorm-user-session.entity';
 ...
 *
 * Since D4 there is no default connection: every entity is overridden under
 * the connection that owns it. A repository that forgets its connection name
 * cannot resolve, and fails here.
 */
describe('AppModule wiring', () => {
  const entities: Array<[new () => unknown, string]> = [
    [TypeOrmUserEntity, 'auth'],
    [TypeOrmUserSession, 'auth'],
  ...
  const build = () => {
    ...
    for (const [entity, connection] of entities) {
      builder
        .overrideProvider(getRepositoryToken(entity, connection))
        .useValue({});
    }
