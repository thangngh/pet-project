...
export class ProductRepository implements IProductRepository {
  ...
  private toTypeOrm(domain: Product): TypeOrmProduct {
    ...
    entity.attributes = domain.attributes.map((a) => ({
      id: a.id,
      name: a.name,
      value: a.value,
    }));
    entity.media = domain.media.map((m) => ({
      id: m.id,
      url: m.url,
      type: m.type,
  ...
  private toDomain(entity: TypeOrmProduct): Product {
    const attrs = (entity.attributes || []).map(
      (a) => new ProductAttribute(a.id, a.name, a.value),
    );
    const media = (entity.media || []).map(
      (m) => new ProductMedia(m.id, m.url, m.type as any, m.isPrimary),
    );
